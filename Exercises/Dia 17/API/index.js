...
app.post("/usuarios/login", (pedido, respuesta) => {
    let emailEncriptado = seguridad.miEncriptado(pedido.body.email);
    let passHashing = seguridad.miHash(pedido.body.pass)
    mySQL.connection.query('SELECT id FROM usuarios WHERE (administrador = 0 AND email = "' + emailEncriptado + '" AND pass = "' + passHashing + '") ' +
...
app.post("/usuarios/create", (pedido, respuesta) => {
    let emailEncriptado = seguridad.miEncriptado(pedido.body.email);
    let passHashing = seguridad.miHash(pedido.body.pass)
    mySQL.connection.query('INSERT INTO usuarios (email, pass, administrador) VALUES ("' + emailEncriptado + '", "' + passHashing + '", 0)', function (error, resultados) {
        if (error)
            throw error;
